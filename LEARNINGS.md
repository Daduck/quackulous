...
- Display regression coverage now includes a GL2 ultrawide windowed path: `scripts/client-module-smoke.ps1` accepts `-CustomWidth`, `-CustomHeight`, and `-Fullscreen`, sets `r_mode=-1` with custom dimensions, and asserts the renderer mode marker (`MODE: -1, <w> x <h> windowed|fullscreen`) from `qconsole.log`. `client_display_ultrawide_windowed_opengl2` currently runs `3440x1440` windowed in smoke (`ctest --preset windows-msvc-debug-smoke`: 8/8 pass).
- Fullscreen mode can override requested `r_customwidth/r_customheight` and use desktop dimensions while still reporting `r_mode=-1`; strict width/height assertions are brittle for fullscreen smoke. Durable fix: `scripts/client-module-smoke.ps1` supports `-ModeRegex` so tests can assert fullscreen mode semantics (`MODE: -1, [0-9]+ x [0-9]+ fullscreen`) without hard-coding desktop resolution. `client_display_fullscreen_opengl2` now runs in smoke and the suite passes 9/9.
