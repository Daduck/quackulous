...
static void GLSL_PrintInfoLog(GLuint object, qboolean developerOnly)
{
	...
	qglGetObjectParameteriv(object, GL_INFO_LOG_LENGTH, &maxLength);
	...
	if (maxLength < 1023)
	{
		qglGetInfoLog(object, maxLength, &maxLength, msgPart);
	...
	else
	{
		...
		qglGetInfoLog(object, maxLength, &maxLength, msg);
...
static void GLSL_PrintShaderSource(GLuint object)
{
	...
	qglGetObjectParameteriv(object, GL_SHADER_SOURCE_LENGTH, &maxLength);

	msg = ri.Malloc(maxLength);

	qglGetShaderSource(object, maxLength, &maxLength, msg);
...
static int GLSL_CompileGPUShader(GLuint program, GLuint *prevShader, const GLchar *buffer, int size, GLenum shaderType)
{
	...
	qglGetObjectParameteriv(shader, GL_COMPILE_STATUS, &compiled);
	if(!compiled)
	{
	...
	if (*prevShader)
	{
		qglDetachObject(program, *prevShader);
		qglDeleteObject(*prevShader);
	}

	// attach shader to program
	qglAttachObject(program, shader);
...
static void GLSL_LinkProgram(GLuint program)
{
	...
	qglGetObjectParameteriv(program, GL_LINK_STATUS, &linked);
	if(!linked)
	{
...
static void GLSL_ValidateProgram(GLuint program)
{
	...
	qglGetObjectParameteriv(program, GL_VALIDATE_STATUS, &validated);
	if(!validated)
	{
		GLSL_PrintInfoLog(program, qfalse);
		ri.Printf(PRINT_ALL, "\n");
...
static void GLSL_ShowProgramUniforms(GLuint program)
{
	...
	qglGetObjectParameteriv(program, GL_ACTIVE_UNIFORMS, &count);
...
void GLSL_DeleteGPUShader(shaderProgram_t *program)
{
	if(program->program)
	{
		...
		{
			qglDetachObject(program->program, program->vertexShader);
			qglDeleteObject(program->vertexShader);
		}

		if (program->fragmentShader)
		{
			qglDetachObject(program->program, program->fragmentShader);
			qglDeleteObject(program->fragmentShader);
		}
