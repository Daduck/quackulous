	...
	*numOutPoints = 0;
	...
	if ( !counts[1] ) {
		*numOutPoints = numInPoints;
		Com_Memcpy( outPoints, inPoints, numInPoints * (int)sizeof(vec3_t) );
		return;
	}
...
void R_AddMarkFragments(int numClipPoints, vec3_t clipPoints[2][MAX_VERTS_ON_POLY],
	...
	*/
	...
	Com_Memcpy( pointBuffer + (*returnedPoints) * 3, clipPoints[pingPong], numClipPoints * (int)sizeof(vec3_t) );
