						  ...
						  int *pUploadWidth, int *pUploadHeight )
{
	...
	scaledBuffer = ri.Hunk_AllocateTempMemory( (int)sizeof( unsigned ) * scaled_width * scaled_height );
...
void R_DeleteTextures( void ) {
	...
	Com_Memset( glState.currenttextures, 0, (int)sizeof( glState.currenttextures ) );
	if ( qglActiveTextureARB ) {
		GL_SelectTexture( 1 );
		qglBindTexture( GL_TEXTURE_2D, 0 );
		GL_SelectTexture( 0 );
