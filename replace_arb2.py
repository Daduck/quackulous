...
def process_file(filepath):
    ...
    def repl_arb_const(match):
        const = match.group(0)
        # exclusions:
        if "BPTC" in const or "INTENSITY32F" in const:
            return const
        return const.replace("_ARB", "")
    content = re.sub(r'\bGL_[A-Z0-9_]+_ARB\b', repl_arb_const, content)
    
    # Specific Object functions rename
    ...
    content = content.replace("GetObjectParameterfvARB", "GetObjectParameterfv")
    content = content.replace("GetAttachedObjectsARB", "GetAttachedObjects")
    content = content.replace("GetInfoLogARB", "GetInfoLog")
    
    # PFNGL object renames (since we renamed the strings, let's keep them matched if needed, but wait)
    # Actually, if we just let it use the ARB typedefs, it's safer. Let's undo my previous PFNGL renaming.
    ...
    # We will manually fix qglActiveTexture
